...
namespace TortoiseSCM
{
    internal sealed class PartialStructureChoiceForm : Form
    {
        ...
        internal PartialStructureChoiceForm(string description, IEnumerable<string> supported, string kind = "")
        {
            ...
            Size = new Size(760, 470); MinimumSize = new Size(660, 430);
            ...
            foreach (string value in options) choices.Items.Add(value == "keep-local" ? "保留本地更改" : value == "take-incoming" ? "采用传入版本" :
                kind == "local-move" ? "使用另一个新名称继续重命名" : kind == "incoming-delete" ? "以新名称重新添加本地文件" : "本地项另存新名称，保留双方");
            ...
            nameRow.Controls.Add(new Label { Text = "本地的新名称：", AutoSize = true, Padding = new Padding(0, 5, 0, 0) }, 0, 0);
            ...
            choices.SelectedIndexChanged += delegate {
                ...
                explanation.Text = Resolution == "take-incoming" ? "将放弃这个冲突项的本地更改，采用已准备的传入状态。原始本地文件保存在恢复目录中。" :
                    Resolution == "keep-local" ? (kind == "local-move" ? "继续本地重命名。仅改名时保留服务器新内容；如果本地也修改过内容，将采用备份中的本地内容。请确认是否需要先手工合并内容。" : "保留本地意图，并将其应用到已准备的服务器状态上。结果仍需单独检查和提交。") :
                    kind == "local-move" ? "将传入文件移动到新的名称，原路径不再保留。仅改名时保留传入内容；本地同时修改时采用本地备份内容。" :
